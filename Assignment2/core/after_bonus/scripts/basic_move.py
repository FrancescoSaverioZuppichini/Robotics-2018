...
class GoCloseTurnAndGoForward(Hook):
    ...
    def turn_until(self, data, name,  name1, name2, not_touch, has_touch, done):
        ...
        print(name, data.range, self.thymio.sensors_cache[name].range)
        if data.range < 0 and self.thymio.sensors_cache[name].range < 10**(-3):
            print('very close')
            data.range = 0.1 # very close
        
        has_touched = data.range < 10**(-3)
        ...
        if has_touched and not self.stable:
            has_touch()
            if self.thymio.sensors_cache[name2].range > 10**(-3):
                self.thymio.sensors_cache[name2].range = 0.01
            e = np.abs(data.range - self.thymio.sensors_cache[name2].range)
            print  e
            self.stable = e < 0.02
...
class BasicThymio:
    ...
    def __init__(self, thymio_name, hooks=[]):
        ...
        print(self.thymio_name + '/cmd_vel')
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        ...
        sys.exit(1)
    # ex1 
    # thymio = BasicThymio(thymio_name, hooks=[EigthWriter(tol=10**(-2))])
    ...
    # rospy.on_shutdown(thymio.freeze_me)
